...
const AIChat: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const handleSend = async () => {
    ...
    if (!apiKey) {
      console.error("Configuration missing: API_KEY is not set.");
      setMessages(prev => [...prev, { 
        role: 'system', 
        text: lang === 'zh' ? "系统配置未完成，请联系技术团队。" : "System configuration incomplete. Please contact the technical team." 
      }]);
      return;
    }
    ...
    try {
      // Create new instance for each request to ensure stability
      ...
      // Reconstruct simple history for context
      ...
      console.error("AI Strategic Interaction Error:", error);
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: lang === 'zh' ? "抱歉，战略服务连接超时。请检查网络或稍后再试。" : "Strategic service connection timeout. Please check your network or try again." 
      }]);
    } finally {
      setIsLoading(false);
    }
